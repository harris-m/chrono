...
var TIME_UNIT = '(' + exports.INTEGER_WORDS_PATTERN + '|[0-9]+|[0-9]+\.[0-9]+|an?(?:\\s*few)?|half(?:\\s*an?)?)\\s*' + '(sec(?:onds?)?|min(?:ute)?s?|h(?:r|rs|our|ours)?|weeks?|days?|months?|years?|fortnights?)\\s*';
var TIME_UNIT_STRICT = '(?:[0-9]+|an?)\\s*' + '(?:seconds?|minutes?|hours?|days?|fortnights?)\\s*';
...
var PATTERN = new RegExp('(\\W|^)' + '(this|next|last|past)\\s*' + '(' + util.INTEGER_WORDS_PATTERN + '|[0-9]+|few|half(?:\\s*an?)?)?\\s*' + '(seconds?|min(?:ute)?s?|hours?|days?|weeks?|months?|years?|fortnights?)(?=\\s*)' + '(?=\\W|$)', 'i');
  ...
  this.extract = function (text, ref, match, opt) {
    ...
    if (match[RELATIVE_WORD_GROUP].match(/day|week|month|year|fortnight/i)) {
      if (match[RELATIVE_WORD_GROUP].match(/day/i)) {
        ...
        date.add(num * 14, 'd'); // We don't know the exact date for next/last week so we imply
        // them
  ...
  this.refine = function (text, results, opt) {
    ...
    results.forEach(function (result) {
      ...
      if (result.tags["ENTimeExpressionParser"] || result.tags["ZHTimeExpressionParser"] || result.tags["FRTimeExpressionParser"] || result.tags["DETimeExpressionParser"]) {
        ...
        if (!contains_tz_abbr && !result.start.isCertain("day") && result.start.isCertain("hour") && result.start.isCertain("minute") && refMoment.isAfter(result.start.dayjs())) {
          var did_shift_by_meridiem = false;

          if (!result.start.isCertain("meridiem")) {
            var result_plus_12_hours = result.start.clone();
            result_plus_12_hours.imply("meridiem", 1);
            result_plus_12_hours.assign("hour", result_plus_12_hours.get("hour") + 12);
